...
from tracking_analyzer.management.commands.install_geoip_dataset import Command
...
class InstallGeoIPDatasetTestCase(TestCase):
    ...
    def test_handle_no_url_arg_no_url_setting(self):
        ...
        present.
        """
        del settings.TRACKING_ANALYZER_MAXMIND_DATABASE

        self.assertRaisesMessage(
            CommandError,
            '`TRACKING_ANALYZER_MAXMIND_DATABASE` setting not present. Unable '
    ...
    def test_handle_no_url_setting_manual_target(self, mock):
        """
        ...
        mock.assert_called_with(
            ...
            mm_dataset=settings.TRACKING_ANALYZER_MAXMIND_DATABASE
        )

    @override_settings(TRACKING_ANALYZER_MAXMIND_DATABASE=None)
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.Command.install_dataset')
    def test_handle_no_database_setting_manual_target(self, mock):
        """
        If the dataset file name is not available, the user should still be
        able to specify it in in the command arguments.
        """
        del settings.TRACKING_ANALYZER_MAXMIND_DATABASE

        call_command('install_geoip_dataset', dataset='countries_db.mmdb')
    ...
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.Command.install_dataset')
    def test_handle_straight_no_user_input_needed(self, mock):
        """
    ...
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.Command.install_dataset')
    def test_handle_existing_dataset_user_action_required_no(self, mock_command, mock_input, mock_isfile):
        """
    ...
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.Command.user_input')
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.Command.install_dataset')
    def test_handle_existing_dataset_user_action_required_yes(self, mock_command, mock_input, mock_isfile):
        """
        Test the command running in a situation where there is an existent
    ...
    def test_install_dataset_urlopen_http_error(self, mock):
        """
    ...
    @mock.patch('tracking_analyzer.management.commands.install_geoip_dataset.urlopen')
    def test_install_dataset_urlopen_url_error(self, mock):
        """
        ``install_dataset`` method is able to handle ``URLError`` exceptions.
        """
        mock.side_effect = URLError(reason='unknown url type')
